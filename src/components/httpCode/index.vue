      ...
      <template #default="{ node }">
        <div>
          ...
          <el-tag
            ...
            color="#ffcc99"
            v-else-if="
              node.label.includes('301') ||
              node.label.includes('302') ||
              node.label.includes('304')
            "
          >
            <span>
              {{ node.label }}
...
const data = [
  ...
  },
  {
    ...
    children: [
      ...
      {
        label:
          "304（未修改、跳转网页）自从上次请求后，请求的网页未修改过。服务器返回此响应时，不会返回网页内容",
      },
      {
        label:
  ...
  },
  {
    ...
    children: [
      ...
      {
        label:
          "401（未授权）请求要求身份验证对于需要登录的网页，服务器可能返回此响应",
      },
      {
      ...
      {
        label:
          "409（冲突）  服务器在完成请求时发生冲突服务器必须在响应中包含有关冲突的信息",
      },
      {
