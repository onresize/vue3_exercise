<template>
  <el-card style="overflow-y: scroll" class="mixinHeight">
    ...
    <h2>
      <el-link href="https://blog.csdn.net/Boale_H/article/details/120288101" target="_blank">关于外链视频请求403报错问题参考：</el-link>
    </h2>
    <!-- 被监听的大视频窗口 -->
    <div class="videoContainerDom" id="videoContainerDom">
      <!-- 传送门到父级 -->
      <teleport to=".conSmallVideo" :disabled="isModel">
        <video class="videoTarget" id="videoTarget" v-if="xgSrc" :src="xgSrc" controls autoPlay></video>
      </teleport>
    </div>
...
onMounted(() => {
  xgSrc.value = "https://ckplayer-video.oss-cn-shanghai.aliyuncs.com/mp4/1_1920x1080.mp4";
  nextTick(() => {
    document.getElementById("videoTarget").volume = 0.2;
  });
  const { stop } = useIntersectionObserver(document.getElementById("videoContainerDom"), ([{ isIntersecting }], observerElement) => {
